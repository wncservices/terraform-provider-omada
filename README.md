...
> **44 resources** (table below) + 6 data sources, each with acceptance tests in
...
| `omada_gateway_bandwidth_control` | singleton, read/update verified live ✅; distinct from the QoS one |
| `omada_portal_access_control` | singleton, read/update verified live ✅; switches only, policy lists preserved |
...
| `omada_dns_proxy` | singleton, read/update verified live ✅; DNS proxy + DoH. The firmware's resolver list is reported read-only and never rewritten; customised resolvers are managed in full |
| `omada_iptv` | singleton, read/update verified live ✅; IGMP proxy + per-port IPTV mode. The gateway's port list is reported read-only and never rewritten |
| `omada_gateway` | gateway device settings, read/update verified live ✅; adopt-only. **Physical port config deliberately excluded** — that is where the WAN lives |
